...
// extract all lines from image with EDLine method
...
#define Z_AXIS 3
	...
	while(ln != ls.end())
	{
		...
		// if we merge two lines,we need refit the lines
		if(flag)
		{
			ln->dir = -1;
			ln->coef.setZero();
...
// only apply to rectangular board
...
bool ImagePolygon::SortLines(std::list<Line2D>& ls, Polygon2D::ConstPtr prev, Polygon2D::Ptr& ply )
{
	...
	// calculate midpoint of every line
	for(auto& it:ls)
	{
	...
	uint32_t id_cors = 0;
	{
		...
		for(uint32_t i=0; i<size_; i++)
		{
			// calculate point to line's distance and direction error
	...
	for(const auto& it: ls)
	{
		uint32_t pindex = (id+id_cors+1)%size_;
...
//    center = center/size_;
	...
	for(uint32_t i=0; i<size_; i++)
	{
        center2point(i) = (center - ply->vertexs.col(i)).norm();	// distance of center to vertex
        center2line(i) = std::abs( center.dot(ply->edges[i].coef) );// distance of center to edge
	}
...
void ImagePolygon::MarkImage(cv::Mat &img, const std::list<Line2D> &ls)
{
    ...
    for (std::size_t i = 0; i < ls.size(); i++)
    {
        ...
        cv::line(img, p0, p1, color, 12 );
        cv::circle(img, p0, 20, color);
        cv::circle(img, p1, 16, color);
