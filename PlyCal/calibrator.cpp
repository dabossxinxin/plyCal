...
//        problem.AddResidualBlock(cost, loss_function_inlier, q.coeffs().data(), p.data());
	...
	//ceres::Problem problem_inliers;
	//ceres::Solver::Summary summary_inliers;
	//ceres::Solver::Options options_inliers;
	//ceres::LocalParameterization* quaternion_local_parameterization_inliers =
	//	new ceres::EigenQuaternionParameterization;
	//for (const auto& ply : polygons_)
	//{
	//	// add inlier points
	//	ceres::CostFunction* cost = new ceres::AutoDiffCostFunction<Point2PolygonError,1,4,3>(
	//		new Point2PolygonError(K_, ply.pc->inliers, ply.img->vertexs) );
	//	problem_inliers.AddResidualBlock(cost, NULL, q.coeffs().data(), p.data());
	//}
	//problem_inliers.SetParameterization(q.coeffs().data(), quaternion_local_parameterization_inliers);
	////options.linear_solver_type = ceres::DENSE_SCHUR;
	//options_inliers.linear_solver_type = ceres::SPARSE_NORMAL_CHOLESKY;
	//options_inliers.max_num_iterations = 10000;
	//options_inliers.function_tolerance = 1.0e-10;
	//// or use all cpu cores
	//options_inliers.num_threads = boost::thread::hardware_concurrency() - 1;
	//options_inliers.num_linear_solver_threads = options_inliers.num_threads;
	////options.minimizer_progress_to_stdout = true;

	//ceres::Solve(options_inliers, &problem_inliers, &summary_inliers);
	//std::cout << summary_inliers.FullReport() << std::endl;
